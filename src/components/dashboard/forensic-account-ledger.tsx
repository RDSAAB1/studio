...
import { ArrowLeft, Printer, TrendingUp, TrendingDown, ShieldCheck, Activity, BarChart3 } from 'lucide-react';
...
interface ForensicAccountLedgerProps {
    title: string;
...
export const ForensicAccountLedger: React.FC<ForensicAccountLedgerProps> = ({ 
    title, 
    icon, 
    data, 
    onBack,
    summary = { inLabel: 'Total Credit', outLabel: 'Total Debit', netLabel: 'Net Balance' }
}) => {
    ...
    const { detailedData, dateWiseData } = useMemo(() => {
        ...
        const dwWithBalance = Object.keys(groups).sort().map(dayKey => {
            ...
            return { 
                ...g, 
                particulars: `Consolidated ${title} for ${format(new Date(g.date), 'dd MMM yyyy')}`,
                id: 'DAILY',
            ...
            <html>
                <head>
                    <style>
                        ...
                        .summary-box { display: flex; justify-content: space-between; align-items: center; gap: 15px; padding: 8px; border: 1pt solid #000; background: #f5f5f5; }
                    </style>
                </head>
                <body>
                    <div class="header">
                        <h1>${title} - ${viewMode === 'DATE_WISE' ? 'Date-wise' : 'Detailed'} Ledger</h1>
                        <p>Forensic Financial Report | Generated on ${format(new Date(), 'dd MMM yyyy HH:mm')}</p>
                    </div>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ${activeData.map(t => `
                                <tr>
                                    ...
                                    <td class="text-right font-bold">${t.balance?.toFixed(2) || '0.00'}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
                    ...
                    <div class="summary-box">
                        <div style="flex: 1;">
                            <span style="font-size: 7pt; color: #666; display: block; text-transform: uppercase;">${summary.inLabel}</span>
                            <span style="font-size: 9pt; font-weight: bold;">₹${totals.in.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div style="flex: 1; border-left: 0.5pt solid #ccc; padding-left: 15px;">
                            <span style="font-size: 7pt; color: #666; display: block; text-transform: uppercase;">${summary.outLabel}</span>
                            <span style="font-size: 9pt; font-weight: bold;">₹${totals.out.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div style="flex: 1.2; border-left: 1pt solid #000; padding-left: 15px; background: #fff; padding: 6px; border-radius: 4px;">
                            <span style="font-size: 7pt; color: #000; display: block; text-transform: uppercase; font-weight: 900;">${summary.netLabel}</span>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1 flex items-center gap-1">
                            <ShieldCheck size={12} className="text-emerald-500" /> Forensic Account Analysis
                        </p>
                    </div>
                </div>
                ...
                <Card className="rounded-3xl border-none shadow-xl shadow-slate-100 bg-white overflow-hidden group">
                    <CardContent className="p-6">
                        <div className="flex items-center gap-2 text-red-600 mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">{summary.outLabel}</span>
                        </div>
                        <div className="text-3xl font-black text-slate-900 font-mono">
                ...
                <div className="bg-slate-50 px-8 py-6 border-b border-slate-100 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-base font-black text-slate-900 uppercase tracking-tight">
                            {viewMode === 'DATE_WISE' ? 'Date-wise Summary' : 'Detailed Transaction Trail'}
                        </h3>
                    </div>
                    ...
                    <Table>
                        <TableHeader className="sticky top-0 z-10 bg-white border-b border-slate-100">
                            <TableRow className="hover:bg-transparent border-none">
                                ...
                                <TableHead className="py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Description</TableHead>
                                ...
                                <TableRow key={idx} className="hover:bg-slate-50/80 transition-colors border-b border-slate-50 group">
                                    ...
                                    <TableCell className="py-5">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">{t.type}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-5">
                                    ...
                                    <TableCell className="text-right py-5 px-8">
                                        <span className="text-[14px] font-mono font-black text-slate-900">
                                            ₹{Math.round(t.balance || 0).toLocaleString('en-IN')}
                                        </span>
                                    </TableCell>
                                </TableRow>
                            ))}
