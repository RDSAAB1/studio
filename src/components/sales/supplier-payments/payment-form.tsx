...
import { Check, ChevronsUpDown, Calendar as CalendarIcon, Settings, Calculator, RefreshCw } from "lucide-react";
...
export const PaymentForm = ({
    ...
    paymentType, setPaymentType, paymentAmount, setPaymentAmount, cdEnabled, setCdEnabled,
    cdPercent, setCdPercent, cdAt, setCdAt, calculatedCdAmount, sixRNo, setSixRNo, sixRDate,
    setSixRDate, utrNo, setUtrNo, checkNo, setCheckNo,
            ...
            <div className="mt-2 space-y-2">
                <Card className="bg-muted/30 p-2">
                    ...
                    <>
                        ...
                        <div className="flex items-center space-x-2 pb-1"><Switch id="cd-toggle" checked={cdEnabled} onCheckedChange={setCdEnabled} /><Label htmlFor="cd-toggle" className="text-xs">Apply CD</Label></div>
                        {cdEnabled && <>
                    ...
                    <div className="mt-2 border-t pt-2 space-y-2">
                        <Card className="p-2 grid grid-cols-1 lg:grid-cols-2 gap-2">
                            <div className="p-2 border rounded-lg space-y-2">
                                <div className="flex justify-between items-center mb-1"><p className="text-xs font-semibold">Bank Details</p><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setIsBankSettingsOpen(true)}><Settings className="h-4 w-4"/></Button></div>
                                <div className="grid grid-cols-2 gap-2 items-end">
                                    ...
                                    <div className="space-y-1"><Label className="text-xs">Branch</Label>
                                        ...
                                        <PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Search branch..." /><CommandEmpty>No branch found.</CommandEmpty><CommandList>{availableBranches.map((branch) => (<CommandItem key={branch.ifscCode} value={branch.branchName} onSelect={(currentValue) => handleBranchSelect(currentValue)}><Check className={cn("mr-2 h-4 w-4", bankDetails.branch === branch.branchName ? "opacity-100" : "opacity-0")} />{branch.branchName}</CommandItem>))}</CommandList></Command></PopoverContent>
                                        </Popover>
                                    </div>
