...
export const Tab: React.FC<TabProps> = ({ icon, title, isActive, onClick, onClose, isClosable }) => {
  const tabClasses = cn(
    "relative flex items-center justify-center cursor-pointer group px-1 sm:px-2 h-[36px] flex-1 transition-colors rounded-[10px] border",
    {
    ...
    >
      <div className={tabClasses}>
        <div className="flex items-center gap-1.5 z-10 overflow-hidden min-w-0 flex-1 justify-center">
          ...
          <span className="whitespace-normal break-words text-center leading-[1.1] text-[10.5px] sm:text-[11.5px]">{title}</span>
        </div>
      </div>
    </div>
  );
