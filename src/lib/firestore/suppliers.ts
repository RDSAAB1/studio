...
import { suppliersCollection, supplierBankAccountsCollection, supplierPaymentsCollection, handleSilentError, createLocalSubscription, stagedSuppliersCollection } from "./core";
...
export async function deleteSupplier(id: string): Promise<void> {
  ...
  try {
    ...
    if (supplierData.brokerageTxId) {
      try {
        const { deleteIncome } = await import('./finance');
        await deleteIncome(supplierData.brokerageTxId);
      } catch (e) {
        console.error('Failed to delete brokerage transaction during supplier delete:', e);
      }
    }
...
export async function deleteMultipleSuppliers(supplierIds: string[]): Promise<void> {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && db) {
      ...
      const brokerageTxIdsToDelete = localSuppliers
        .map(s => s.brokerageTxId)
        .filter(Boolean) as string[];

      if (brokerageTxIdsToDelete.length > 0) {
        const { deleteMultipleIncomes } = await import('./finance');
        try {
          await deleteMultipleIncomes(brokerageTxIdsToDelete);
        } catch (e) { }
      }
    ...
    if (db) {
      const { isLocalFolderMode, removePaymentsFromFolderFile, removeMultipleRecordsFromFolderFile } = await import('@/lib/local-folder-storage').catch(() => ({ isLocalFolderMode: () => false, removePaymentsFromFolderFile: async () => false, removeMultipleRecordsFromFolderFile: async () => false }));
      ...
      if (isLocalFolderMode() && validSupplierIds.length > 0) {
        await removeMultipleRecordsFromFolderFile('suppliers', validSupplierIds, 'id').catch(() => { });
      }
  ...
  for (const id of supplierIds) {
    ...
    if (supplierData) {
      ...
      const updatePayload = {
        ...
        finalAmount: (recalculatedData as any).finalAmount,
        brokerage: (recalculatedData as any).brokerage,
        cd: (recalculatedData as any).cd,
      };
...
export async function addStagedSupplier(supplierData: Customer): Promise<Customer> {
  try {
    const documentIdValue = supplierData.id || String(Date.now() + Math.random());
    const supplierWithCorrectId = { ...supplierData, id: documentIdValue };
    const { writeLocalFirst } = await import('../local-first-sync');
    const result = await writeLocalFirst('staged_suppliers', 'create', documentIdValue, supplierWithCorrectId) as Customer;

    if (typeof window !== 'undefined' && result) {
      window.dispatchEvent(new CustomEvent('indexeddb:collection:changed', {
        detail: { collection: 'staged_suppliers' },
      }));
    }
    return result;
  } catch (error) {
    logError(error, `addStagedSupplier(${supplierData.id})`, 'high');
    throw error;
  }
}

export async function updateStagedSupplier(id: string, supplierData: Partial<Omit<Customer, 'id'>>): Promise<boolean> {
  if (!id) return false;
  try {
    const { writeLocalFirst } = await import('../local-first-sync');
    await writeLocalFirst('staged_suppliers', 'update', id, undefined, supplierData as Partial<Customer>);
...
export async function deleteStagedSupplier(id: string): Promise<void> {
  if (!id) return;
  try {
    const dbRaw = getDb();
    await dbRaw.stagedSuppliers.delete(id);
    const { writeLocalFirst } = await import('../local-first-sync');
    await writeLocalFirst('staged_suppliers', 'delete', id);
    ...
    logError(error, `deleteStagedSupplier(${id})`, 'high');
    throw error;
  }
}

export async function deleteMultipleStagedSuppliers(ids: string[]): Promise<void> {
  if (!ids || ids.length === 0) return;
  try {
    const dbRaw = getDb();
    await dbRaw.stagedSuppliers.bulkDelete(ids);
    ...
    if (isLocalFolderMode()) {
      await syncCollectionToFolder('staged_suppliers').catch(() => { });
    }
    ...
    logError(error, `deleteMultipleStagedSuppliers`, 'high');
    throw error;
  }
}

export async function bulkUpsertStagedSuppliers(suppliers: Customer[], chunkSize = 400) {
  if (!suppliers.length) return;
  const chunks = chunkArray(suppliers, chunkSize);
  if (!isSqliteMode()) {
    for (const chunk of chunks) {
      ...
      chunk.forEach((supplier) => {
        if (!supplier.id) throw new Error("Supplier entry missing id");
        const ref = doc(stagedSuppliersCollection, supplier.id);
        batch.set(ref, supplier, { merge: true });
      });
      await batch.commit();
    }
...
export function getStagedSuppliersRealtime(
  callback: (data: Customer[]) => void,
  onError: (error: Error) => void
): () => void {
  return createLocalSubscription<Customer>("staged_suppliers", callback);
}

export async function mergeStagedSuppliers(selectedStaged: Customer[]): Promise<{ addCount: number; updateCount: number }> {
    ...
    // Fetch existing suppliers to check for updates vs inserts
    const existingSuppliers = await dbRaw.suppliers.where('srNo').anyOf(targetSrNos).toArray();
    const existingMap = new Map(existingSuppliers.map(s => [s.srNo, s]));

    const suppliersToPut: Customer[] = [];
    ...
    for (const staged of selectedStaged) {
        ...
        if (existing) {
            ...
            // Add new supplier
            const documentIdValue = targetSrNo && targetSrNo.trim() !== '' && targetSrNo !== 'S----' 
                ? targetSrNo 
        ...
        suppliersToPut.push(finalRecord);
        stagedIdsToDelete.push(staged.id);
    }

    // Perform bulk operations in Dexie/IndexedDB
    if (suppliersToPut.length > 0) {
        await dbRaw.suppliers.bulkPut(suppliersToPut);
    }
    if (stagedIdsToDelete.length > 0) {
        await dbRaw.stagedSuppliers.bulkDelete(stagedIdsToDelete);
    }
    ...
    try {
        ...
        if (isLocalFolderMode()) {
            for (const record of suppliersToPut) {
                await mergeRecordToFolderFile('suppliers', record as unknown as Record<string, unknown>, 'id').catch(() => {});
            }
            await syncCollectionToFolder('staged_suppliers').catch(() => {});
        }
    } catch (e) {
        // Ignore folder storage import/execution errors if they occur
    }

    // Dispatch collection changed events exactly once at the end
    if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('indexeddb:collection:changed', { detail: { collection: 'suppliers' } }));
        window.dispatchEvent(new CustomEvent('indexeddb:collection:changed', { detail: { collection: 'staged_suppliers' } }));
    }
